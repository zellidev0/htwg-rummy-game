...
class Controller(var desk: DeskInterface) extends ControllerInterface {
  ...
  override def nameInputFinished(): Unit =
    if (desk.correctAmountOfPlayers) {
      ...
      switchState(AnswerState.INSERTING_NAMES_FINISHED, ControllerState.P_TURN)
  ...
  override def loadFile(): Unit = fileIO.load match {
    case Some(value) =>
      desk = value
      switchState(AnswerState.LOADED_FILE, ControllerState.P_TURN)
    case None =>
      switchState(AnswerState.COULD_NOT_LOAD_FILE, ControllerState.MENU)
      createDesk(12)
  }
  ...
  override def createDesk(amount: Int): Unit = {
    ...
    desk = Desk(List[PlayerInterface](), Random.shuffle(bagOfTiles), Set[SortedSet[TileInterface]]())
