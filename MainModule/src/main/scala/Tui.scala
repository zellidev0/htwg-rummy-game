...
class Tui(var controller: ControllerInterface) extends UIInterface {

  override def processInput(input: String): ControllerInterface =
    printOut(controller.state match {
  ...
  private def handleOnTurn(input: String): ControllerInterface = input match {
    case LayDownTilePattern(c) => controller.layDownTile(Tile.stringToTile(c.split(" ").apply(1)).get);
  ...
  private def printCurrentStateView(currentState: ControllerState.Value): Unit =
    println(currentState match {
      case ControllerState.P_TURN =>
        s"""
           |---------------------------------------------------------------------------------------|
           | ${controller.currentPlayerName} it's your turn. Do your stuff.${" " * (55 - controller.currentPlayerName.length)}|
           |---------------------------------------------------------------------------------------|
  ...
  private def printAnswerState(answerState: AnswerState.Value): Unit =
    println(answerState match {
      ...
      case AnswerState.P_WON => s"${controller.currentPlayerName} is the winner."
  ...
  def printCurrentBoardView(board: SortedSet[TileInterface]): Unit = {
    var s =
      s"""
         |---------------------------------------------------------------------------------------|
         | ${controller.currentPlayerName} thats on your board.${" " * (65 - controller.currentPlayerName.length)}|
         |---------------------------------------------------------------------------------------|""" + "\n"
    for (_ <- board) {
      s += "____ "
    }
