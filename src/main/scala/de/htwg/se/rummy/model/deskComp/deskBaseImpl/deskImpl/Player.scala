...
case class Player @Inject()(name: String, number: Int, board: BoardInterface, state: state = State.WAIT) extends PlayerInterface {
  ...
  override def removeTile(tile: TileInterface): PlayerInterface =
    copy(board = board - tile)

  override def addTile(tile: TileInterface): PlayerInterface =
    copy(board = board + tile)

  override def changeState(newState: State.Value): PlayerInterface =
    copy(state = newState)

  override def toString: String =
    s"Player number: $number, state: ${state.toString}, boardsize: ${board.amountOfTiles()}"

  override def hasTile(tile: TileInterface): Boolean =
    board.contains(tile)
