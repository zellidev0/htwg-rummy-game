...
class PlayerSpec() extends WordSpec with Matchers {
  "A Player" when {
    "created" should {
      ...
      "have a name" in {
        player.getName should be("Name1")
      }
    ...
    "its status is changed" should {
      ...
      "the new status be turn " in {
        player1.getTiles.size should be(0)
      }
    }
